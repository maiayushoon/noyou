  ...
  return (
    <section className="relative overflow-hidden bg-gradient-to-b from-brand-50 via-white to-white">
      ...
      <div aria-hidden="true" className="pointer-events-none absolute inset-0 overflow-hidden">
        <motion.div
          className="absolute -top-32 left-1/4 h-[28rem] w-[28rem] rounded-full bg-indigo-300/30 blur-3xl"
          animate={reduceMotion ? undefined : { y: [0, 24, 0], x: [0, 16, 0] }}
          transition={reduceMotion ? undefined : { duration: 14, repeat: Infinity, ease: "easeInOut" }}
        />
        <motion.div
          className="absolute -right-24 top-10 h-[24rem] w-[24rem] rounded-full bg-cyan-300/25 blur-3xl"
        ...
        />
        <motion.div
          className="absolute bottom-0 left-0 h-[22rem] w-[22rem] rounded-full bg-violet-300/25 blur-3xl"
          animate={reduceMotion ? undefined : { y: [0, -18, 0] }}
          transition={reduceMotion ? undefined : { duration: 18, repeat: Infinity, ease: "easeInOut" }}
        />
        {/* Subtle grid wash for depth. */}
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_top,rgba(99,102,241,0.05),transparent_55%)]" />
      </div>
      ...
      <div className="relative mx-auto grid max-w-content items-center gap-12 px-4 pb-20 pt-16 sm:px-6 sm:pt-20 lg:grid-cols-[1.05fr_1fr] lg:gap-8 lg:px-8 lg:pt-24">
        ...
        >
          <motion.span
            variants={item}
            className="inline-flex items-center gap-2 rounded-full border border-indigo-200 bg-white/80 px-3 py-1 text-xs font-semibold text-indigo-700 shadow-sm backdrop-blur"
          >
          ...
          <motion.h1
            variants={item}
            className="mt-6 text-balance text-4xl font-bold tracking-tight text-brand-900 sm:text-5xl lg:text-6xl"
          >
            Own your online and{" "}
          ...
          <motion.p
            variants={item}
            className="mx-auto mt-6 max-w-xl text-balance text-lg leading-relaxed text-slate-600 sm:text-xl lg:mx-0"
          >
          ...
          >
            ...
            <a
              href="/features"
              className="inline-flex w-full items-center justify-center rounded-xl border border-slate-300 bg-white/80 px-6 py-3.5 text-base font-semibold text-slate-800 backdrop-blur transition-colors hover:border-indigo-300 hover:text-indigo-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 sm:w-auto"
            >
              See how it works
            </a>
          </motion.div>
      ...
      <div className="relative mx-auto max-w-content px-4 pb-16 sm:px-6 lg:px-8">
        <motion.dl
          ...
          className="mx-auto grid max-w-5xl grid-cols-2 gap-px overflow-hidden rounded-2xl border border-slate-200 bg-slate-200 shadow-card sm:grid-cols-4"
        >
          {STATS.map((stat) => (
            <motion.div
              key={stat.label}
              variants={item}
              className="group bg-white p-5 text-center transition-colors hover:bg-indigo-50/50"
            >
              <dt className="sr-only">{stat.label}</dt>
              <dd>
                <span className="block bg-gradient-to-br from-indigo-600 to-cyan-500 bg-clip-text text-2xl font-bold text-transparent">
                  {stat.value}
                </span>
